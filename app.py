...
st.title("🧬 ImmuneEQ")
st.subheader("CD4/CD8 Trajectories, Treatment Heterogeneity & Equity-Informed Precision Medicine in HIV Care")
...
def train_model(df):
    ...
    return rf, feature_cols, X_test, y_test

with st.spinner("Loading dataset and training model..."):
    df = load_data()
    rf, feature_cols, X_test, y_test = train_model(df)
...
race_options = st.sidebar.multiselect("Race", options=['White', 'Non-white'], default=['White', 'Non-white'])
gender_options = st.sidebar.multiselect("Gender", options=['Male', 'Female'], default=['Male', 'Female'])
risk_options = st.sidebar.multiselect("Transmission Risk Group", options=df['risk_group'].unique().tolist(), default=df['risk_group'].unique().tolist())

trt_vals = [k for k, v in trt_map.items() if v in trt_options]
race_vals = [0 if r == 'White' else 1 for r in race_options]
...
col4.metric("Mean CD4 at 20wks", f"{filtered['cd420'].mean():.0f} cells/mm³")
...
with tab4:
    ...
    st.markdown("Does the model perform equally well across demographic groups?")
